...
// the fields the ops hub reads or writes.
// ---------------------------------------------------------------------------

export type PhoenixRuntimeMode = "approval-required" | "auto-accept-edits" | "auto" | "full-access";

/** Only "default" is ever sent by the ops hub; "plan" exists for interactive UI use. */
 ...
 * `worktreePath`/`branch` are always null: the ops hub launches root
  ...
  async function request(
    ...
    try {
      response = await fetch(`${baseUrl}${path}`, {
        method: init.method,
        headers: {
          Authorization: `Bearer ${config.PHOENIX_OPS_TOKEN}`,
