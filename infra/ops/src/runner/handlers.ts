...
// infra/ops/src/runner/handlers.ts -> package root (infra/ops) is two
// directories up.
const PACKAGE_ROOT = fileURLToPath(new URL("../../", import.meta.url));

// `workflow.skill_path` is stored relative to the workflows root
// (`OPS_WORKFLOWS_DIR`), not the package root directly — confirmed against
// src/workflows/loader.ts's `WorkflowDefinition.skillPath` doc comment
// ("relative to the workflows root"). `OPS_WORKFLOWS_DIR` itself resolves
...
function resolveWorkflowsRoot(): string {
  return isAbsolute(config.OPS_WORKFLOWS_DIR)
    ? config.OPS_WORKFLOWS_DIR
    : resolve(PACKAGE_ROOT, config.OPS_WORKFLOWS_DIR);
}
...
export function createWorkflowLaunchHandler(deps: {
  ...
  return defineJobHandler({
    ...
    async handle(job: OpsJobRow, lease: ActiveJobLease) {
      ...
      const callbackUrl = `${config.OPS_PUBLIC_URL.replace(/\/+$/, "")}/api/runs/${runId}/result`;
      ...
      await enqueueJob({
        ...
        runAfter: new Date(Date.now() + config.OPS_RUN_WATCH_INTERVAL_MS),
      });
