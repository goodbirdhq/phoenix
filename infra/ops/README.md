...
This assumes the ops hub is running alongside Phoenix on the same box, the
way it does in production today.

### Pairing with Phoenix

The ops hub launches agent threads through Phoenix's HTTP API, which needs a
bearer token. Mint one once (see `docs/phoenix-http-contract.md` for the
full contract):

```sh
phoenix pair --label "ops-hub"
```
...
curl -s -X POST "$PHOENIX_BASE_URL/oauth/token" \
  ...
  -d client_label=ops-hub-service
```

The response is `{access_token, token_type: "Bearer", expires_in: 2592000,
scope}` — a token good for 30 days.

The token lives in `~/.phoenix/userdata/secrets/ops-hub-token`, as a single
`PHOENIX_OPS_TOKEN=<access_token>` line — that file doubles as the
...
```sh
OPS_DATABASE_URL="postgresql://projectx:<password>@localhost:5433/phoenix_ops"
PHOENIX_BASE_URL="http://100.68.224.90:3874"
PHOENIX_OPS_TOKEN="<from pairing, above>"
...
curl -s "$PHOENIX_BASE_URL/api/orchestration/snapshot" \
  -H "Authorization: Bearer $PHOENIX_OPS_TOKEN" | jq
```

Look for the project whose `workspaceRoot` is this box's ops/business repo
checkout.

Everything else in `.env.example` (`OPS_HTTP_PORT`, `OPS_SCHEDULER_TICK_MS`,
`OPS_TIMEZONE`, `OPS_WORKFLOWS_DIR`, `OPS_RUN_TIMEOUT_MS`,
`OPS_RUN_WATCH_INTERVAL_MS`) has a sane default; only override on the box if
you have a specific reason to.

### Database

The ops hub's Postgres lives in the shared `projectx-postgres` container
(`localhost:5433`, user `projectx`), not a dedicated instance — it's a
database inside that container, not a separate container:

```sh
createdb -h localhost -p 5433 -U projectx phoenix_ops
```
...
3. `schedules` is a list of cron triggers, each `{cron, timezone, enabled}`
   (`timezone` defaults to `OPS_TIMEZONE`, `enabled` defaults to `true`).
   ...
   (`OPS_SCHEDULER_TICK_MS`, 15s by default) — editing a manifest or adding
...
- **`shadow`** — a real agent thread runs, but the ops hub tells it (in the
...
```sh
pnpm --dir infra/ops exec tsx src/cli.ts run <key> --input '{"prospect": {"company": "Acme"}}'
```
...
Both print/return a `runId`; `pnpm --dir infra/ops exec tsx src/cli.ts list`
...
```ini
# /etc/systemd/system/ops-hub.service
[Unit]
Description=Phoenix ops hub worker
After=network.target postgresql.service

[Service]
Type=simple
WorkingDirectory=/path/to/phoenix/infra/ops
EnvironmentFile=/home/<user>/.phoenix/userdata/secrets/ops-hub-token
EnvironmentFile=/path/to/phoenix/infra/ops/.env.production
ExecStart=/usr/bin/pnpm exec tsx src/cli.ts worker
# (WorkingDirectory above makes this equivalent to `pnpm --dir infra/ops exec tsx src/cli.ts worker`)
Restart=on-failure
RestartSec=5
...
Two `EnvironmentFile`s: the token file (just `PHOENIX_OPS_TOKEN=...`,
...
`JSON.stringify({event, ...})`) — `journalctl -u ops-hub -f | jq` to follow
...
- **HTTP bind is loopback-only.** `startHttpServer` binds `127.0.0.1`
  regardless of `OPS_HTTP_PORT` (`src/http/server.ts`) — the callback
...
- **The manual-trigger route (`POST /api/workflows/:key/run`) has no
  ...
  can reach `127.0.0.1:$OPS_HTTP_PORT` on this box can start any enabled
