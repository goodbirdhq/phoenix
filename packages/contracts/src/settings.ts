...
export const RemoteWorkspaceRoot = TrimmedNonEmptyString.check(
  ...
  Schema.isPattern(/^\/(?:[^/]+(?:\/|$))*$/),
  Schema.makeFilter(
    (value: string) => !value.includes(String.fromCharCode(0)) || "Path cannot contain NUL.",
  ),
);
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Phoenix spawn the server when needed.",
        providerSettingsForm: {
...
export const ServerSettings = Schema.Struct({
  ...
  // Lets an agent session spawn and orchestrate sibling sessions through the
...
export const ClientSettingsPatch = Schema.Struct({
  ...
  sidebarAttentionFirstEnabled: Schema.optionalKey(Schema.Boolean),
