...
`,
        ...
        const explicitlyConfigured = mode === "node-override";
        assert.equal(exitCode, explicitlyConfigured ? 0 : 1);
        if (explicitlyConfigured) {
          assert.deepEqual(decodeArguments(stdout), args);
          assert.equal(stderr, "");
        } else {
          assert.equal(stdout, "");
          assert.include(stderr, "Phoenix has an owned package distribution");
        }
