...
#[zbus::interface(name = "com.goodbird.Phoenix.KdeCapture")]
impl ScriptReply {
    fn reply(
        &self,
        value: &str,
...
fn script(connection: &Connection, directory: &Path, body: &str) -> Result<String> {
    ...
    connection.object_server().at(
        "/com/goodbird/Phoenix/KdeCapture",
        ScriptReply {
            owner,
            result: send,
        },
    ...
    let source = format!(
        "function reply(value) {{ callDBus({destination}, '/com/goodbird/Phoenix/KdeCapture', 'com.goodbird.Phoenix.KdeCapture', 'Reply', JSON.stringify(value)); }}\ntry {{ {body} }} catch (error) {{ reply({{error: String(error)}}); }}"
    );
    ...
    connection
        .object_server()
        .remove::<ScriptReply, _>("/com/goodbird/Phoenix/KdeCapture")?;
