...
import type { ApnsNotificationPayload } from "./payloads.ts";

export function isTerminalPhase(state: AgentActivityState): boolean {
...
export function agentActivityExpiresAt(
  state: Pick<AgentActivityState, "phase" | "updatedAt">,
): number {
...
export function isExpiredAgentActivityState(state: AgentActivityState, nowMs: number): boolean {
...
export function sanitizeAgentActivityAggregateState(
  aggregate: AgentActivityAggregateState,
): AgentActivityAggregateState {
  return {
    ...aggregate,
