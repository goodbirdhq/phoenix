import type { AgentActivityAggregateState } from "@t3tools/contracts/notifications";
...
import { ApnsEnvironment as ApnsEnvironmentSchema, type ApnsCredentials } from "./configuration.ts";
import type { ApnsLiveActivityAlert, ApnsNotificationPayload } from "./payloads.ts";
...
function contentState(state: AgentActivityAggregateState) {
  return {
...
type MakeLiveActivityRequestInput =
  | (LiveActivityRequestBase & {
      readonly event: "end";
      readonly state: AgentActivityAggregateState | null;
      readonly alert?: ApnsLiveActivityAlert | null;
    })
  | (LiveActivityRequestBase & {
      readonly event: "start" | "update";
      readonly state: AgentActivityAggregateState;
      readonly alert?: ApnsLiveActivityAlert | null;
    });
...
export const make = Effect.gen(function* () {
  ...
  const sendLiveActivityRequest: ApnsClient["Service"]["sendLiveActivityRequest"] = Effect.fn(
    "notifications.apns.send_live_activity_request",
  )(function* (input) {
    yield* Effect.annotateCurrentSpan({ "notifications.apns.event": input.request.event });
  ...
  const sendPushNotificationRequest: ApnsClient["Service"]["sendPushNotificationRequest"] =
    Effect.fn("notifications.apns.send_push_notification_request")(function* (input) {
      yield* Effect.annotateCurrentSpan({ "notifications.apns.event": "push_notification" });
      const jwt = yield* providerTokens.getJwt({
        ...input.credentials,
        issuedAtUnixSeconds: input.issuedAtUnixSeconds,
      });
