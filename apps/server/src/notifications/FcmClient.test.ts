...
import { FcmConfiguration } from "./configuration.ts";
...
} satisfies FcmConfiguration["Service"];
const input = {
  token: "device-token",
...
function testLayer(
  ...
  responses: Response[],
  configuration: FcmConfiguration["Service"] = config,
) {
  ...
  return layer.pipe(
    ...
    Layer.provide(
      Layer.mergeAll(
        Layer.succeed(FcmConfiguration, configuration),
...
describe("FCM delivery", () => {
  it.effect("reports missing delivery credentials before any network request", () => {
    const requests: HttpClientRequest.HttpClientRequest[] = [];
    return Effect.gen(function* () {
      const client = yield* FcmClient;
      const error = yield* client.send(input).pipe(Effect.flip);
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.provide(
          layer.pipe(
            ...
            Layer.provide(Layer.succeed(FcmConfiguration, config)),
