...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `phoenix connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `phoenix connect link` to enable T3 Connect."
      : !status.linked
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `phoenix connect unlink` again when the relay is reachable.",
      );
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `phoenix connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `phoenix connect link` before the
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\nPhoenix is set to run while you are logged in to this Mac. The server establishes the T3 Connect link on startup."
              : "\n✓ Background service ready\n\nPhoenix is set to keep running after you log out. The server establishes the T3 Connect link on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
