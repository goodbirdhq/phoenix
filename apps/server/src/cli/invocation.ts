 ...
 * Detection is best-effort; callers must fail closed to a plain `phoenix` command.
 */
...
/**
 * The `phoenix` package spec to suggest. The literal spec the user typed (e.g.
 * `phoenix@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "phoenix@nightly" : "phoenix";
}

/**
 * Render a `phoenix <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx phoenix connect` suggests
 * `npx phoenix serve`, a global install suggests `phoenix serve`, and a nightly build
