...
it("points a stale service at the published update command", () => {
  assert.include(
    ...
    "Next: Run `npx @goodbirdhq/phoenix@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx @goodbirdhq/phoenix@0.0.32-nightly.1 service update");
  expect(output).not.toContain("@goodbirdhq/phoenix@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx @goodbirdhq/phoenix@0.0.29 service update");
  expect(output).not.toContain("npx @goodbirdhq/phoenix@0.0.28 service update");
});
