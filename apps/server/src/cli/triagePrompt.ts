/**
 * All text `phoenix triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Phoenix triage playbook

You are a support engineer for Phoenix (https://github.com/goodbirdhq/phoenix), working
...
Fetch https://raw.githubusercontent.com/goodbirdhq/phoenix/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/goodbirdhq/phoenix <source-cache-dir>/<hash>
...
- How is Phoenix running on this machine: \`phoenix serve\` in a terminal, the
...
Search existing issues in goodbirdhq/phoenix (use \`gh\`, or the public GitHub search
...
Do not patch the Phoenix source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/goodbirdhq/phoenix/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Phoenix triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Phoenix user is \
having a problem with their install and started this session with \`phoenix triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Phoenix triage context

Generated by \`phoenix triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/goodbirdhq/phoenix
