...
export const SettleSessionTool = Tool.make("settle_session", {
  description:
    "Mark a session this session spawned as finished, so it stops showing up as live work. This STOPS the child's agent process if it is still alive — settling is declaring the child done, so the process goes with it. A child that is mid-turn (starting or running) is refused instead: call stop_session yourself if you really mean to interrupt live work. Optionally pass cleanupWorktree: true to PERMANENTLY DELETE the child's git worktree directory and its temporary Phoenix branch; this cannot be undone, and is refused (with the specific dirty files and unpushed commit count) when the worktree still holds work that is not committed and pushed, unless you also pass force: true. Add cleanupBranch: true to also delete a branch you named yourself, which happens only when Phoenix can prove it was merged (local head == remote head == the head commit of a merged pull request) and is otherwise refused with the SHAs that disagree. Cleanups on one repository run one at a time, so parallel settles queue instead of fighting over git's lock. The result always lists exactly what was removed, what was kept, and the proof used; a warning field appears when the child's process outlived its stop.",
