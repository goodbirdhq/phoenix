...
const published = { checkedAt, windows: [session, weekly], planKind: "subscription" as const };
...
describe("applyUsageLimitsUpdate", () => {
  ...
  it("clears a stale probe failure when a runtime update supplies a valid window", () => {
    const failedProbe = {
      ...published,
      unavailable: { reason: "probeFailed" as const },
    };
    const next = applyUsageLimitsUpdate({
      previous: failedProbe,
      checkedAt: "2026-09-03T12:00:05.000Z",
      update: { windows: [{ ...session, usedPercent: 55 }] },
    });
