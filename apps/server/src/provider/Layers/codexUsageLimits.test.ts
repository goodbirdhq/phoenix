...
it("classifies Codex billing only from native plan slugs", () => {
  expect(codexPlanKind("plus")).toBe("subscription");
  expect(codexPlanKind("enterprise")).toBe("enterprise");
  expect(codexPlanKind("enterprise_cbp_usage_based")).toBe("paygo");
  expect(codexPlanKind("self_serve_business_usage_based")).toBe("paygo");
  expect(codexPlanKind("unknown")).toBe("unknown");
  expect(codexPlanKind(undefined)).toBe("unknown");
  expect(codexPlanKind("unrecognized-plan")).toBe("unknown");
});
