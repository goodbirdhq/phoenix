...
export function codexPlanLabel(planType: string | null | undefined): string | undefined {
  switch (planType) {
    ...
    case "self_serve_business_usage_based":
      return "ChatGPT Business Pay-as-you-go";
    case "ent26":
    case "enterprise":
      return "ChatGPT Enterprise Subscription";
    case "enterprise_cbp_automation":
    case "enterprise_cbp_usage_based":
      return "ChatGPT Enterprise Pay-as-you-go";
    case "edu":
    case "edu_plus":
    case "edu_pro":
    ...
    if (!codexSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Codex is disabled in Phoenix settings.",
        },
      });
    }
...
> {
  ...
  const accountPlanType =
    snapshot.account.account?.type === "chatgpt" ? snapshot.account.account.planType : undefined;
  const usageLimits: ServerProviderUsageLimits =
    snapshot.account.account?.type === "apiKey"
      ? { ...makeUnavailableUsageLimits({ checkedAt, reason: "unsupported" }), planKind: "unknown" }
      : snapshot.rateLimits === undefined || "failure" in snapshot.rateLimits
        ? {
            ...
            planKind: codexPlanKind(accountPlanType),
          }
        : codexRateLimitsToLimits({
            snapshot: {
              ...snapshot.rateLimits.snapshot,
              planType: snapshot.rateLimits.snapshot.planType ?? accountPlanType ?? null,
            },
