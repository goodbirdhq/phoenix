...
/** Billing classification is native-plan evidence only; unknown slugs stay unknown. */
export function codexPlanKind(planType: string | null | undefined): ProviderPlanKind {
  const normalized = planType?.trim().toLowerCase();
  if (!normalized || normalized === "unknown") return "unknown";
  if (normalized.includes("usage_based") || normalized.includes("cbp")) return "paygo";
  if (normalized === "enterprise" || normalized === "ent26") return "enterprise";
  if (
    [
      "free",
      "go",
...
export function codexRateLimitsToLimits(input: {
  ...
  return {
    ...
    planKind: codexPlanKind(input.rateLimitsByLimitId?.codex?.planType ?? input.snapshot.planType),
