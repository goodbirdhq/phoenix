...
it.effect("ProviderServiceLive streams the cached availability after passive updates", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      assert.equal(update.instanceId, codexInstanceId);
      assert.equal(update.provider, CODEX_DRIVER);
      assert.equal(update.availability.status, "available");
      assert.equal(update.availability.source, "codex_app_server");
      assert.equal(update.availability.observedAt, "2026-08-16T12:00:00.000Z");
      assert.deepEqual(
        update.availability.windows.map((window) => ({
          kind: window.kind,
          label: window.label,
...
it.effect(
  ...
  () =>
    Effect.gen(function* () {
      ...
      // A normalized Agent SDK update with no quota rows must not blank the
...
describe("agent browser access", () => {
  ...
  it.effect("issues a credential without preview when agent browser access is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["pull-requests", "schedules", "sessions"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [
        { threadId, capabilities: ["device", "preview", "pull-requests", "schedules", "sessions"] },
      ]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("drops only the preview capability when browser access alone is off", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["device", "pull-requests", "schedules", "sessions"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
  ...
  it.effect("requests an MCP credential when the project overrides browser access to on", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(issued, [{ threadId, capabilities: ["preview", "pull-requests", "schedules", "sessions"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
