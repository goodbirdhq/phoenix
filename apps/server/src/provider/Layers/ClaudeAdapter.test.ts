    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        harness.query.interrupt = () =>
          Effect.runPromise(Deferred.succeed(entered, undefined).pipe(Effect.asVoid));
        yield* adapter.startSession({
          threadId: THREAD_ID,
          provider: ProviderDriverKind.make("claudeAgent"),
          runtimeMode: "full-access",
  ...
  it.effect("acceptForSession returns session-scoped permission updates", () => {
    ...
    return Effect.gen(function* () {
      ...
      const mcpPermissionPromise = canUseTool(
        ...
        { title: "hello" },
        {
          requestId: "permission-request-mcp-1",
          signal: new AbortController().signal,
          suggestions: [],
      ...
      const bashPermissionPromise = canUseTool(
        ...
        { command: "git status" },
        {
          requestId: "permission-request-bash-1",
          signal: new AbortController().signal,
          suggestions: [
            {
              type: "addRules",
  ...
  it.effect("routes Claude resume compaction through the shared user-input UI", () => {
    ...
    return Effect.gen(function* () {
      ...
      const dialogPromise = onUserDialog(
        ...
        { signal: new AbortController().signal, requestId: "resume-return-request" },
      );
    ...
    return Effect.gen(function* () {
      ...
      const permissionPromise = canUseTool(
        ...
        },
        {
          ...
          requestId: "tool-ask-pre-aborted-request",
        },
      );
  ...
  it.effect("stopping a session settles pending user-input waits", () => {
    ...
    return Effect.gen(function* () {
      ...
      const permissionPromise = canUseTool(
        ...
        {
          requestId: "permission-request-ask-stop",
          signal: new AbortController().signal,
