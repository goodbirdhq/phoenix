...
const claudeAvailabilityFromRateLimitEvent = (
  ...
  const windows = event.payload.limits.windows;
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const agentAccessCapabilities = Effect.fn("ProviderService.agentAccessCapabilities")(function* (
    ...
    // Sessions and schedules always attach: each toolkit enforces its own
    // dynamic server setting (enableSessionOrchestration/enableScheduleManagement)
    // at the tool boundary rather than gating the shared credential itself.
  ...
  const processRuntimeEvent = (
    ...
    const ingest = Effect.gen(function* () {
      ...
      if (threadId !== undefined) {
        ...
        if (directory.recordActivity) {
          yield* directory.recordActivity(threadId, source.instanceId, observedAt);
        }
      }
      const availability = availabilityFromRuntimeEvent(canonicalEvent);
      if (availability) {
        const now = yield* DateTime.now;
        yield* cacheAvailability(
          source.instanceId,
          source.provider,
          availability,
