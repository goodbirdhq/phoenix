...
const buildUserMessageEffect = Effect.fn("buildUserMessageEffect")(function* (
  ...
  const finalText = dependencies.seedPrompt
  ...
  if (dispatch) {
    ...
    sdkContent.push({ type: "text", text: finalText });
  }
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleResultMessage = Effect.fn("handleResultMessage")(function* (
    ...
    const status = successShapedUsageLimit ? "failed" : outcome.status;
    const errorMessage = successShapedUsageLimit ? resultText : outcome.errorMessage;
    const errorKind =
      status === "failed" &&
  ...
  const handleSdkTelemetryMessage = Effect.fn("handleSdkTelemetryMessage")(function* (
    ...
    if (message.type === "rate_limit_event") {
      ...
      if (rateLimitInfo.status === "rejected" && context.turnState) {
        context.turnState.usageLimitSignaled = true;
      }
    ...
    function* (input) {
      ...
      const queryOptions: ClaudeQueryOptions = {
        ...
        env: McpProviderSession.withAgentDeviceEnvironment(
          { ...claudeEnvironment, [T3_THREAD_ID_ENV_VAR]: input.threadId },
          mcpSession,
        ),
