...
sessionErrorLayer("CodexAdapterLive session errors", (it) => {
  it.effect("passes MCP session capabilities into the runtime", () => {
    ...
    return Effect.gen(function* () {
      ...
      NodeAssert.deepEqual(runtime?.options.mcpCapabilities, new Set(["preview"]));
    }).pipe(
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("labels MCP lifecycle entries with server and tool names", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.equal(firstEvent.value.payload.title, "phoenix · preview_status");
...
for (const accountType of ["chatgpt", "apiKey"] as const)
  it.effect(
    `refreshes ${accountType} quotas in the configured Codex home without starting a thread`,
    () =>
      Effect.gen(function* () {
...
require("node:readline").createInterface({input:process.stdin}).on("line", line => {
  ...
  const result = message.method === "initialize" ? {userAgent:"codex/test",codexHome:process.env.CODEX_HOME,platformFamily:"unix",platformOs:"linux"}
    : message.method === "account/read" ? {account:process.env.PROBE_ACCOUNT_TYPE === "apiKey" ? {type:"apiKey"} : {type:"chatgpt",email:"test@example.invalid",planType:"pro"},requiresOpenaiAuth:true}
...
`,
        ...
        const adapter = yield* makeCodexAdapter(
          decodeCodexSettings({ binaryPath: binary, homePath }),
          {
            instanceId: ProviderInstanceId.make("codex-b"),
            environment: {
              CODEX_HOME: "/wrong-account",
              PROBE_LOG: log,
              PROBE_ACCOUNT_TYPE: accountType,
            },
          },
        ).pipe(
          Effect.provide(
        ...
        if (accountType === "chatgpt") NodeAssert.equal(reading.windows[0]?.usedPercent, 37);
        else
          NodeAssert.deepEqual(
            { source: reading.source, windows: reading.windows },
            { source: "unsupported", windows: [] },
          );
