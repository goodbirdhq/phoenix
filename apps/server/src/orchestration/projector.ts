...
function retainPendingQuestionActivities(activities: OrchestrationThread["activities"]) {
  const recentStart = activities.length - MAX_THREAD_NON_REPORT_ACTIVITIES;
...
export function projectEvent(
  ...
  switch (event.type) {
    ...
    // A migration rebinds the thread to the target instance. The history row
