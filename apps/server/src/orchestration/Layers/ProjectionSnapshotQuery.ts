...
import { ProjectionThreadReport } from "../../persistence/Services/ProjectionThreadReports.ts";
...
const ProjectionThreadMessageDbRowSchema = ProjectionThreadMessage.mapFields(
  Struct.assign({
    ...
    origin: Schema.NullOr(Schema.fromJsonString(OrchestrationMessageOrigin)),
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listThreadReportRows = SqlSchema.findAll({
    Request: Schema.Void,
  ...
  const listQueuedTurnStartRowsByThread = SqlSchema.findAll({
    ...
    execute: ({ threadId, messageId }) =>
      sql`
        SELECT message_id AS "messageId", thread_id AS "threadId", turn_id AS "turnId", role, text,
          attachments_json AS "attachments", origin_json AS "origin", context_json AS "context",
          is_streaming AS "isStreaming", created_at AS "createdAt",
          updated_at AS "updatedAt"
        FROM projection_thread_messages
        WHERE thread_id = ${threadId} AND message_id = ${messageId}
        LIMIT 1
  ...
  const listThreadReportRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadReportDbRowSchema,
    execute: ({ threadId }) =>
      sql`
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listThreadReportRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listThreadReports:query",
                "ProjectionSnapshotQuery.getSnapshot:listThreadReports:decodeRows",
              ),
            ),
          ),
          ...
          listQueuedTurnStartRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listQueuedTurnStarts:query",
                "ProjectionSnapshotQuery.getSnapshot:listQueuedTurnStarts:decodeRows",
              ),
            ),
          ),
        ]),
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              const reportsByThread = new Map<string, Array<SessionReport>>();
              for (const row of reportRows) {
                const threadReports = reportsByThread.get(row.threadId) ?? [];
                threadReports.push(mapReportRow(row));
                reportsByThread.set(row.threadId, threadReports);
              }
              ...
              for (const row of queuedTurnStartRows) {
                ...
                snapshotQueuedByThread.set(row.threadId, rows);
              }
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                reportDelivery: row.reportDelivery,
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadReportRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadReports:query",
                "ProjectionSnapshotQuery.getCommandReadModel:listThreadReports:decodeRows",
              ),
            ),
          ),
          listThreadPullRequestRows(undefined).pipe(
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        reportDelivery: threadRow.value.reportDelivery,
        ...mapThreadPullRequests(
  ...
  const getThreadDetailByIdBounded = (
    ...
    Effect.gen(function* () {
      ...
      const [
        ...
        // Undelivered queue state travels with the detail so clients can mark
        // messages the agent has not consumed yet; the reducer keeps it live
        // from the turn-start-queued/requested/consumed/cancelled events.
        listQueuedTurnStartRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listQueuedTurnStarts:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listQueuedTurnStarts:decodeRows",
            ),
          ),
        ),
      ]);
      ...
      const thread = {
        ...
        queuedTurnStarts: queuedTurnStartRows.map((row) => ({
          messageId: row.messageId,
          mode: row.mode,
          requestedAt: row.requestedAt,
          ...(row.releasingAt !== null ? { releasingAt: row.releasingAt } : {}),
