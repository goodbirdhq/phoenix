...
describe("ProviderRuntimeIngestion", () => {
  ...
  async function createHarness(options?: {
    ...
    const projectionTurns = await testRuntime.runPromise(Effect.service(ProjectionTurnRepository));
    ...
    return {
      ...
      readShell: () =>
        testRuntime.runPromise(snapshotQuery.getThreadShellById(asThreadId("thread-1"))),
      readThreadShell: () =>
        testRuntime.runPromise(
          snapshotQuery
  ...
  it.each(["releasing", "cancelled", "requeued", "requeued-cancelled"])(
    ...
    async (recovery) => {
      ...
      await harness.dispatch({
        ...
        threadId,
        message: { messageId, role: "user", text: "authorization", attachments: [] },
        runtimeMode: "approval-required",
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        createdAt: now,
      });
      ...
      await harness.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("native-running"),
        threadId,
        session,
        createdAt: now,
      });
      ...
      expect(await harness.readQueuedDeliveryReceipts()).toContainEqual(
        expect.objectContaining({
          messageId,
          state: "consumed",
          consumedByTurnId: asTurnId("accepted-turn"),
          cancelledAt: null,
        }),
      );
