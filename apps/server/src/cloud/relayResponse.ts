...
function recoveryHint(error: RelayProtectedError): string {
  switch (error._tag) {
    case "RelayEnvironmentLinkLimitExceededError":
      return "Unlink an unused environment in T3 Connect, then restart Phoenix on this machine.";
    case "RelayAuthInvalidError":
      return "Run `phoenix connect login` to check this machine's authorization. If the stored credential was revoked, sign out with `phoenix connect logout`, then run `phoenix connect` again. Restart Phoenix after signing in.";
    case "RelayEnvironmentLinkProofExpiredError":
    case "RelayEnvironmentLinkProofInvalidError":
      return "Check this machine's date and time, update Phoenix, then restart it.";
    default:
...
export const filterRelayResponse = Effect.fn("cloud.filter_relay_response")(function* (
  ...
  const message = Option.isSome(decoded)
    ...
    : `T3 Connect relay returned HTTP ${response.status} without a recognized error response. Check relay access and any proxy or firewall restrictions, then restart Phoenix.${requestId}`;
