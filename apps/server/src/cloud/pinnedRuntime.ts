...
import { PUBLISHED_PACKAGE_NAME, PUBLISHED_PACKAGE_PATH_SEGMENTS } from "./serviceProtocol.ts";

/**
 * A pinned runtime is an exact `@goodbirdhq/phoenix@<version>` npm-installed
 * into <baseDir>/runtime/versions/<version>. The boot service points its unit
 * or launch agent here, and server self-update installs the target version
 * here before switching over — never `npx`, whose cache is ephemeral and whose
...
/**
 * Installs `@goodbirdhq/phoenix@<version>` into the pinned runtime directory
 * unless a complete install is already there, and returns its paths. The
 * sentinel is written only after npm exits 0; checking the entry file alone is
 * not enough. npm extracts files before running native builds (node-pty), so a
 * killed install leaves a plausible-looking but broken tree behind.
 */
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned Phoenix runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
