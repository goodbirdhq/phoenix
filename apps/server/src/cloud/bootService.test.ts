...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const baseDir = path.join(home, ".phoenix");
  ...
  const runtime = yield* pinVersion("1.2.3");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable phoenix.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart phoenix.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop phoenix.service" ||
  ...
  return { service, makeService, pinVersion, fs, statePath, commands, timeouts, control, runtime };
});
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop phoenix.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now phoenix.service")).toEqual(
  ...
  it.effect("reports the installed runtime when this CLI targets an older version", () =>
    Effect.gen(function* () {
      const { service, pinVersion, fs, statePath } = yield* makeHarness();
      yield* service.install();
      yield* pinVersion("1.2.4");
      yield* fs.writeFileString(
        statePath,
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop phoenix.service", "systemctl --user restart phoenix.service"]
            : [
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.goodbird.phoenix.service.plist"),
        ),
      ).toBe(true);
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.goodbird.phoenix.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.goodbird.phoenix.service",
        "launchctl enable gui/501/com.goodbird.phoenix.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.goodbird.phoenix.service";
