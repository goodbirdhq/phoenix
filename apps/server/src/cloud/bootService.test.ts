...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  return { service, makeService, pinVersion, fs, statePath, commands, timeouts, control };
});
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith("Library/LaunchAgents/com.goodbird.phoenix.service.plist"),
      ).toBe(true);
