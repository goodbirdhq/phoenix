...
// `phoenix service update`. Keep runtime imports limited to Node built-ins.
...
async function main(): Promise<void> {
  const baseDir = process.env.PHOENIX_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("PHOENIX_HOME is required by the Phoenix service launcher.");
  }
