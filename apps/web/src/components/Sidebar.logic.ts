...
/** A successful explicit Active reorder is the user's choice of Manual ordering. A failed write
 * leaves Attention ordering intact, so a transient save error cannot make priority work disappear. */
export function resolveSidebarAttentionFirstAfterActiveReorder(input: {
  readonly attentionFirstEnabled: boolean;
  readonly reorderSucceeded: boolean;
}): boolean {
  return input.reorderSucceeded ? false : input.attentionFirstEnabled;
}
