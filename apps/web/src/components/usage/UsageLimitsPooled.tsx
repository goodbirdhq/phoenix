...
function planKindLabel(planKind: LimitAccount["limits"]["planKind"]): string | null {
  switch (planKind) {
    case "subscription":
      return "Subscription";
    case "paygo":
...
function SegmentPopover({
  ...
  const planKind = planKindLabel(account.limits.planKind);
  return (
    <div className="flex w-72 max-w-[calc(100vw-3rem)] flex-col gap-2.5 text-xs">
      ...
      <div className="flex flex-col gap-1 border-t border-border/60 pt-2.5">
        ...
        {planKind ? <Row label="Plan type">{planKind}</Row> : null}
        {where ? (
