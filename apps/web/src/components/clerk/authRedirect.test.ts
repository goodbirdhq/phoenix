  ...
  it("removes a Clerk virtual pathname and callback params while preserving the desktop route", () => {
    expect(
      resolveClerkSignInProps(
        "phoenix://app/CLERK-ROUTER/VIRTUAL/sign-up?__clerk_status=complete#/settings/connections",
        true,
      ),
    ).toEqual({
      forceRedirectUrl: "phoenix://app/#/settings/connections",
      signUpForceRedirectUrl: "phoenix://app/#/settings/connections",
    });
  });

  it("preserves a clean development desktop route", () => {
    expect(resolveClerkSignInProps("phoenix-dev://app/#/settings/general", true)).toEqual({
      forceRedirectUrl: "phoenix-dev://app/#/settings/general",
      signUpForceRedirectUrl: "phoenix-dev://app/#/settings/general",
    });
  });
});
