...
const SessionMessageCtaRow = memo(function SessionMessageCtaRow(props: {
  ...
  const content = (
    <>
      ...
      />
      <WorkEntryIcon name="message-circle" className="size-3.5 shrink-0 text-muted-foreground" />
      <span className="min-w-0 truncate">
        <span className="font-medium">{lead}</span>
        {message.preview ? (
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const entryIconName = retryState
    ? retryState === "exhausted"
      ? "circle-alert"
      : "refresh-cw"
    : showWarningIndicator || (showDestructiveRowStyle && !toolPresentation)
      ? "circle-alert"
      : workEntryIconName(workEntry);
  const entryToolIcon =
    retryState || showWarningIndicator || showDestructiveRowStyle
      ? undefined
