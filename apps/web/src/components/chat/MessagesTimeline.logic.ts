...
export function deriveMessagesTimelineRows(input: {
  ...
  const activeTurnHasVisibleContent = activeEntries.some((entry) => {
    ...
    if (entry.kind === "work") {
      return (
        !workEntryIsSpawnCta(entry.entry) &&
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      ...
      if (visibleGroupedEntries.length > 0) {
        // `workEntryIsSpawnCta` is the single answer to "must this row stay
        // visible": it covers agent spawns plus the spawned-session and
        // Schedule receipts, which would otherwise collapse into a
        // "+N tool calls" toggle and hide the one row that mattered.
        const onlyToolEntries = visibleGroupedEntries.every(
          (entry) =>
            workLogEntryIsToolLike(entry) && !workEntryIsSpawnCta(entry) && entry.tone !== "error",
        );
