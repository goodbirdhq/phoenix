...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps an in-progress spawned-session card outside work-log overflow", () => {
    ...
    // The spawn card is its own row: it breaks the live tool run rather than
    // being swept into the collapsed `work-live` group beside it.
    expect(rows.map((row) => row.id)).toEqual([
      "working-indicator-row",
      "work-1",
      "spawn-session",
      "work-live:work-entry-3",
    ]);
  ...
  it("keeps a spawned-session card visible when its launching turn folds", () => {
    ...
    // The card survives the fold; the fold marker anchors after the rows the
    // fold cannot claim.
    expect(rows.map((row) => row.id)).toEqual([
      "user-entry",
      "spawn-session-entry",
