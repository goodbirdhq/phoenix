...
function isRemoteWorkspaceRoot(value: string): boolean {
  return !value.includes(String.fromCharCode(0)) && /^\/(?:[^/]+(?:\/|$))*$/.test(value);
}

export function EnvironmentEditorHandoffRow({
  environment,
  title,
}: {
  ...
  const [sshHostAlias, setSshHostAlias] = useState(
    handoff.mode === "vscode-remote-ssh" ? handoff.sshHostAlias : "",
  );
  const [sourceWorkspaceRoot, setSourceWorkspaceRoot] = useState(
    handoff.mode === "vscode-remote-ssh" ? handoff.sourceWorkspaceRoot : "",
  );
        ...
        <div className="grid gap-2 px-3 pb-3 sm:grid-cols-2 sm:px-4">
          ...
          {invalidRemoteConfig ? (
            <p className="text-xs text-destructive sm:col-span-2">
              Enter a host alias and both source and Remote-SSH workspace roots as absolute POSIX
              paths.
            </p>
          ) : null}
        </div>
...
type SavedBackendListRowProps = {
  ...
  onDisconnect: (environmentId: EnvironmentId) => Promise<void>;
...
export function ConnectionsSettings({ hostOnly = false }: { hostOnly?: boolean } = {}) {
  ...
  const disconnectEnvironment = useAtomCommand(
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. Phoenix restarts when you change this."
  ...
  return (
    <SettingsPageContainer embedded={hostOnly}>
      {canManageLocalBackend ? (
        <>
          <SettingsSection
            ...
            {...(hostOnly
              ? {
                  ...
                  description: "Manage how your other devices connect to this environment.",
                }
              : {})}
          >
          ...
          {!hostOnly && isLocalBackendRemotelyReachable ? (
            <SettingsSection
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Phoenix will restart to expose this environment over the network."
                    : "Phoenix will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Phoenix will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Phoenix until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Phoenix will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Phoenix will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Phoenix will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Phoenix will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Phoenix will restart the local backend with Tailscale Serve enabled and ask
          ...
          >
            {savedEnvironments.map((environment) => (
              <SavedBackendListRow
                ...
                onDisconnect={async (environmentId) => {
                  await disconnectEnvironment(environmentId);
                }}
              />
            ))}
