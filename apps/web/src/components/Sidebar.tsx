...
import { SessionIcon, SidebarTeamAvatars } from "./sidebar/SidebarTeamAvatars";
import {
  buildSidebarTeams,
  visibleSidebarTeams,
...
} from "./sidebar/SidebarTeam.logic";
        ...
        <div className="grid gap-1.5 pl-0.5 text-xs text-muted-foreground">
          ...
          {parentTitle ? (
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate">From {parentTitle}</span>
            </div>
          ) : null}
          {environmentLabel ? (
            <div className="flex min-w-0 items-center gap-2">
            ...
            <PopoverTrigger
              render={
                <button
                  ...
                  className="inline-flex size-6 cursor-pointer items-center justify-center rounded-md bg-transparent text-xs text-muted-foreground hover:text-foreground"
                />
              }
            />
          }
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  const isWoke = hasUnseenSidebarWake(props.wokeAt, lastVisitedAt);
  // Status hues follow the system-wide convention set by sidebar v1 and the
  // mobile Live Activity/widgets (amber approval, indigo input, sky working)
  // so a thread reads the same color everywhere it surfaces.
  const reviewAction =
    status === "approval"
      ? { label: "Review decision", className: "text-[#B45309] dark:text-amber-300" }
      : status === "input"
        ? { label: "Review input", className: "text-[#4F46E5] dark:text-indigo-300" }
        : status === "failed"
  ...
  const rowSurfaceClassName = cn(
    "group/sidebar-row relative w-full cursor-pointer rounded-lg text-left outline-none select-none focus-visible:ring-2 focus-visible:ring-ring",
    props.isActive
  ...
  const title = isRenaming ? (
    ...
    <span
      className={cn(
        "min-w-0 flex-1 text-sm leading-5 transition-opacity motion-reduce:transition-none group-hover/sidebar-row:pr-[72px] group-focus-within/sidebar-row:pr-[72px]",
        "truncate",
      ...
      <Tooltip>
        <TooltipTrigger render={<span className="inline-flex shrink-0" />}>
          <a
            ...
            className={cn(
              "inline-flex shrink-0 rounded-sm focus-visible:ring-2 focus-visible:ring-ring",
              prStatus.colorClass,
            )}
  ...
  return (
    <li
      ...
      style={{
        ...
        ...(isNested
          ? ({
              paddingLeft: `min(${(props.hierarchyIndentDepth - 1) * 20}px, max(0px, calc(100% - 244px)))`,
              // The first rail aligns with the parent card; deeper rails step 20px.
              "--sidebar-hierarchy-rail": `min(${(props.hierarchyIndentDepth - 1) * 20}px, max(0px, calc(100% - 244px)))`,
            } as CSSProperties)
          : undefined),
      }}
      {...(sortable?.listeners ?? {})}
      className={cn(
        ...
        isNested
          ? "relative [contain-intrinsic-size:auto_66px]"
      ...
      >
        <div className="relative flex min-h-[66px] items-center gap-2.5 px-2.5 py-3">
          <Tooltip
            ...
            onOpenChange={(open) => {
              if (!open) setAvatarDetailsOpen(false);
            }}
          >
            <TooltipTrigger
              render={
                <button
                  ...
                  aria-label={
                    reviewAction
                      ? `${reviewAction.label} in ${teamStatus.target.title}`
                      : "Session details"
                  }
                  className="flex w-8 shrink-0 items-center rounded-full focus-visible:ring-2 focus-visible:ring-ring"
                  onClick={(event) => {
                    event.stopPropagation();
                    if (reviewAction)
                      onThreadActivate(
                    ...
                    else setAvatarDetailsOpen((open) => !open);
                  }}
                />
              }
            >
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <div className="relative flex h-5 min-w-0 items-center gap-2 [@media(pointer:coarse)]:pr-8">
              ...
              <span
                className={cn(
                  "w-[30px] shrink-0 text-right text-xs leading-[18px] text-sidebar-muted-foreground group-hover/sidebar-row:invisible group-focus-within/sidebar-row:invisible",
              ...
              >
                {variant === "slim" ? (
                  <button
                    type="button"
                    onClick={
                      variantAction === "unsnooze" ? handleUnsnoozeClick : handleUnsettleClick
                    }
                    aria-label={
                      variantAction === "unsnooze" ? "Wake thread now" : "Un-settle thread"
                    }
                    disabled={
                      variantAction === "unsnooze"
                    ...
                    className="px-1 text-xs text-sidebar-muted-foreground hover:text-sidebar-foreground disabled:hidden"
                  >
                    {variantAction === "unsnooze" ? "Wake now" : "Unsettle"}
                  </button>
                ) : props.settlementSupported ? (
                  <Tooltip>
                    <TooltipTrigger
            ...
            <div className="flex h-[18px] min-w-0 items-center gap-1.5 text-xs leading-[18px] text-sidebar-muted-foreground">
              {prBadge}
...
export default function Sidebar() {
  ...
  const visibleTeamRows = useMemo(
    () =>
      sessionHierarchyEnabled
        ? visibleSidebarTeams(teamThreads, expandedTeamKeys)
  ...
  const { activeThreadRows, pinnedChildRows } = useMemo(() => {
    ...
    const pinned = new Map<string, typeof visibleTeamRows>();
    let pinnedRoot: string | null = null;
    for (const row of visibleTeamRows) {
      if (row.depth === 0)
        pinnedRoot = row.thread.pinnedAt != null ? sidebarTeamKey(row.thread) : null;
      if (row.thread.pinnedAt != null) continue;
      if (pinnedRoot === null) active.push(row);
      else {
        const children = pinned.get(pinnedRoot);
        if (children) children.push(row);
        else pinned.set(pinnedRoot, [row]);
      }
    }
    return { activeThreadRows: active, pinnedChildRows: pinned };
  }, [visibleTeamRows]);

  // Reveal a thread opened through search, a review action, or a deep link.
  // Explicitly collapsing it stays respected until navigation changes again.
  const revealedRouteRef = useRef<string | null>(null);
  useEffect(() => {
    if (!sessionHierarchyEnabled || routeThreadKey === null) {
      revealedRouteRef.current = null;
      return;
    }
    if (revealedRouteRef.current === routeThreadKey) return;
    const byKey = new Map(teamThreads.map((thread) => [sidebarTeamKey(thread), thread]));
    let current = byKey.get(routeThreadKey);
    if (!current) return;
    revealedRouteRef.current = routeThreadKey;
    const ancestors = new Set<string>();
    while (current?.spawnedByThreadId && current.pinnedAt == null) {
      const parentKey = `${current.environmentId}:${current.spawnedByThreadId}`;
      if (ancestors.has(parentKey) || parentKey === routeThreadKey || !byKey.has(parentKey)) break;
      ancestors.add(parentKey);
      current = byKey.get(parentKey);
    }
    if (ancestors.size > 0) setExpandedTeamKeys((previous) => new Set([...previous, ...ancestors]));
  }, [routeThreadKey, sessionHierarchyEnabled, teamThreads]);
  ...
  const orderedThreads = useMemo(
    () => [
      ...visibleTeamRows.map((row) => row.thread),
      ...visibleSnoozedThreads,
      ...renderedSettledThreads,
    ],
    [visibleTeamRows, visibleSnoozedThreads, renderedSettledThreads],
  );
  ...
  const planForwardNavigation = useCallback(
    (threadKey: string, coParkingKeys?: ReadonlySet<string>): (() => void) | null => {
      ...
      const anchor = sidebarNavigationAnchor(threadKey, orderedKeys, threadByKeyRef.current);
      const currentIndex = anchor === null ? -1 : orderedKeys.indexOf(anchor);
      const nextCardKey =
        currentIndex === -1
          ? null
      ...
      if (traversalDirection !== null) {
        navigateToThreadKey(
          resolveAdjacentThreadId({
            ...
            currentThreadId: sidebarNavigationAnchor(
              routeThreadKey,
              orderedThreadKeys,
              threadByKey,
            ),
    ...
    <>
      ...
      <SidebarContent
        ...
        fixedHeader={
          ...
          <SidebarGroup className="relative z-[1] mt-2 h-[52px] pl-5 pr-6 py-2.5 pointer-coarse:py-1">
        ...
        <SidebarGroup className="pl-[15px] pr-3.5 pb-1 pt-2">
          ...
          {!isSearchingThreads ? (
            <TooltipProvider
              ...
              timeout={400}
            >
              <ul ref={attachListAutoAnimateRef} role="list" className="flex flex-col gap-2">
                {(() => {
                  const renderThreadRow = (
                    ...
                    // Lifecycle selects the section and actions, never the row geometry.
                      ...
                      <SidebarThreadRow
                        ...
                        parentTitle={
                          thread.spawnedByThreadId
                            ? (parentTitleByKey.get(
                                `${thread.environmentId}:${thread.spawnedByThreadId}`,
                              ) ?? null)
                            : null
                        }
                  ...
                  // Drafts precede pinned and recent sections; empty sections have no heading.
                  ...
                  if (activeThreadRows.length > 0)
                    items.push(
                      <li
                        key="recent-heading"
                        className="flex items-center gap-1.5 px-2.5 pt-3 text-xs leading-[18px] text-sidebar-muted-foreground"
                      >
                    ...
                    items.push(
                      ...
                      >
                        <button
                          ...
                          className="flex w-full cursor-pointer items-center gap-1.5 border-t border-sidebar-border px-2.5 py-3 text-left leading-[18px]"
                        >
                          <ClockIcon className="size-3.5 text-sidebar-muted-foreground" />
                          <span className="flex-1 text-xs text-sidebar-muted-foreground">
                            Snoozed · {snoozedThreads.length}
                          </span>
                          <ChevronDownIcon
                            aria-hidden
                            className={cn(
                              "size-3 -rotate-90 text-sidebar-muted-foreground transition-transform",
                              snoozedShelfExpanded && "rotate-0",
                            )}
                          />
                        </button>
                      </li>,
                    ...
                    items.push(
                      ...
                      >
                        <button
                          ...
                          className="flex w-full cursor-pointer items-center gap-1.5 border-t border-sidebar-border px-2.5 py-3 text-left leading-[18px]"
                        >
                          <CircleCheckIcon className="size-3.5 text-sidebar-muted-foreground" />
                          <span className="flex-1 text-xs text-sidebar-muted-foreground">
