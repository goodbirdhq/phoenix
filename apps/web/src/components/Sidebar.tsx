...
export default function Sidebar() {
  ...
  const filterScopedProjectKeys = useMemo(() => {
  ...
  const comboboxScopedProjectKeys = useMemo(
    () =>
  ...
  // The header combobox narrows to one project; the filter menu can narrow
  // further by environment/status/account/model. Both scopes apply together.
  const scopedProjectKeys = useMemo(() => {
    if (filterScopedProjectKeys === null) return comboboxScopedProjectKeys;
    if (comboboxScopedProjectKeys === null) return filterScopedProjectKeys;
    return new Set(
      [...filterScopedProjectKeys].filter((key) => comboboxScopedProjectKeys.has(key)),
    );
  }, [filterScopedProjectKeys, comboboxScopedProjectKeys]);
  ...
  // Hierarchy mode hoists a pinned thread's children out of the active list
  // (they render nested under the pinned card instead); the flat list still
  // owes them a spot, so they fall in with the rest of the active rows here.
  const hierarchyActiveRows = useMemo(
    () => [...activeThreadRows, ...pinnedChildRows.values()].flat(),
    [activeThreadRows, pinnedChildRows],
  );
  const hierarchyIndentDepthByKey = useMemo(() => {
    const map = new Map<string, number>();
    for (const row of hierarchyActiveRows) {
      map.set(sidebarTeamKey(row.thread), row.indentDepth);
    }
    return map;
  }, [hierarchyActiveRows]);
  const sidebarListItems = useMemo((): readonly SidebarListItem[] => {
    ...
    const activeRows = sessionHierarchyEnabled
      ? rowsOf(
          hierarchyActiveRows.map((row) => row.thread),
          "active",
        )
  ...
  const handleThreadDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      void (async () => {
        ...
        let activeReorderSucceeded = false;
        for (const assignment of keyWrites) {
          ...
          if (
            ...
            return;
          activeReorderSucceeded ||= plan.kind === "move-active";
        }
        if (
          !resolveSidebarAttentionFirstAfterActiveReorder({
            attentionFirstEnabled,
            reorderSucceeded: activeReorderSucceeded,
          })
        ) {
          // Persisted key writes are the source of truth. A failed lifecycle or key write returns
          // above, leaving Attention active rather than exposing a partial manual arrangement.
          void updateClientSettings({ sidebarAttentionFirstEnabled: false });
        }
      })();
    },
    [
                      ...
                      const renderThreadRowInner = (
                        ...
                        return (
                          <SidebarThreadRow
                            ...
                            thread={thread}
                            variant={rowVariant}
                            hierarchyIndentDepth={hierarchyIndentDepthByKey.get(threadKey) ?? 0}
                            teamMembers={
                              isCard && sessionHierarchyEnabled
                                ? teamsByKey.get(threadKey)
                                : undefined
                            }
                            teamExpanded={
