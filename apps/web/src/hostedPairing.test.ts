...
describe("hostedPairing", () => {
  ...
  it.each(["", "  ", "not-a-url", "javascript:alert(1)"])(
    "does not generate hosted links without a valid configured URL: %s",
    (value) => {
      vi.stubEnv("VITE_HOSTED_APP_URL", value);
      expect(
        buildHostedPairingUrl({ host: "https://backend.example.com", token: "secret" }),
      ).toBeNull();
  ...
  it("reads hosted pairing host and query token parameters", () => {
    const url = new URL("https://app.phoenix.example/pair?host=100.64.1.2:3773&token=ABCD1234");
  ...
  it("prefers hash tokens so generated hosted links do not put credentials in search params", () => {
    ...
    expect(url.origin).toBe("https://preview.phoenix.example");
  ...
  it("builds hosted channel selection URLs through the configured router origin", () => {
    vi.stubEnv("VITE_HOSTED_APP_URL", "https://app.phoenix.example");
    ...
    expect(url.origin).toBe("https://app.phoenix.example");
  ...
  it("ignores incomplete hosted pairing requests", () => {
    expect(
      hasHostedPairingRequest(new URL("https://app.phoenix.example/pair?host=backend.example.com")),
    ).toBe(false);
    expect(
      hasHostedPairingRequest(new URL("https://app.phoenix.example/pair?token=ABCD1234")),
    ).toBe(false);
  });
  ...
  it("detects the hosted static app only when no backend URL is configured", () => {
    ...
    expect(isHostedStaticApp(new URL("https://preview.phoenix.example/"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://preview.phoenix.example/pair"))).toBe(true);
    expect(isHostedStaticApp(new URL("https://backend.example.com/"))).toBe(false);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://preview.phoenix.example/"))).toBe(false);
  });
  ...
  it("detects hosted channel aliases as static apps", () => {
    ...
    expect(isHostedStaticApp(new URL("https://nightly.app.phoenix.example/"))).toBe(true);

    vi.stubEnv("VITE_HTTP_URL", "https://backend.example.com");
    expect(isHostedStaticApp(new URL("https://nightly.app.phoenix.example/"))).toBe(false);
  });
});
