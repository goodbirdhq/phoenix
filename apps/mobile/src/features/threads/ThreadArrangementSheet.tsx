                    ...
                    {thread ? (
                      <>
                        ...
                        <DragHandle
                          ...
                          onStep={(direction) => {
                            void moveThread(thread, direction).then((moved) => {
                              if (
                                moved &&
                                shouldUseManualOrderingAfterMove(item.section, item.section)
                              )
                                savePreferences({ sidebarAttentionFirstEnabled: false });
                            });
                          }}
                          onStart={() => start(item)}
                          onMove={update}
                          onEnd={(cancelled) => {
                            ...
                            void moveThread(current.thread, current.destination)
                              .then((moved) => {
                                const targetSection = current.destination?.section;
                                if (
                                  moved &&
