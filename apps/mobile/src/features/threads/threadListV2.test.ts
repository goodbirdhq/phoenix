...
import {
  ...
  shouldUseManualOrderingAfterMove,
...
describe("cross-section thread drops", () => {
  ...
  it("selects Manual ordering only after an in-place Active arrangement", () => {
    expect(shouldUseManualOrderingAfterMove("active", "active")).toBe(true);
    expect(shouldUseManualOrderingAfterMove("pinned", "pinned")).toBe(false);
    expect(shouldUseManualOrderingAfterMove("settled", "active")).toBe(false);
    expect(shouldUseManualOrderingAfterMove("active", "settled")).toBe(false);
  });
});
