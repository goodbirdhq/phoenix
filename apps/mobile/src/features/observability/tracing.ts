...
class MobileClientTracer extends Context.Reference(
  "@t3tools/mobile/observability/MobileClientTracer",
  {
    defaultValue: () => Option.none<Tracer.Tracer>(),
  },
) {}

export const withMobileClientTracing = <A, E, R>(
  effect: Effect.Effect<A, E, R>,
): Effect.Effect<A, E, R> =>
  MobileClientTracer.pipe(
    Effect.flatMap(
      Option.match({
        onNone: () => effect,
...
export function makeTracingLayer(
  ...
  if (config === null) {
    return Layer.succeed(MobileClientTracer, Option.none());
  }
  ...
  const tracerLayer = OtlpTracer.layer({
    ...
    resource: {
      ...
      attributes: {
        ...
        "service.component": "mobile-client",
