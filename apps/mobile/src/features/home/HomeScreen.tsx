...
interface HomeScreenProps {
  ...
  readonly onMoveThread: ThreadListActions["moveThread"];
...
export function HomeScreen(props: HomeScreenProps) {
  ...
  const handleUnsnoozeThread = useCallback(
    (thread: EnvironmentThreadShell) => props.onUnsnoozeThread(thread),
    [props.onUnsnoozeThread],
  );
  const handlePinThread = useCallback(
    (thread: EnvironmentThreadShell) => props.onPinThread(thread),
    [props.onPinThread],
  );
  ...
  const handleUnpinThread = useCallback(
    (thread: EnvironmentThreadShell) => props.onUnpinThread(thread),
    [props.onUnpinThread],
  );
  const handleRegenerateThreadTitle = useCallback(
    (thread: EnvironmentThreadShell) => props.onRegenerateThreadTitle(thread),
    ...
    ({ item }: { readonly item: ThreadListV2ListItem }) => {
      ...
      return (
        <ThreadListV2Row
          ...
          canMovePinnedUp={arrangedPinnedKeys.indexOf(movedId) > 0}
          canMovePinnedDown={(() => {
            const index = arrangedPinnedKeys.indexOf(movedId);
      ...
      <View className={materialYouStyleLayoutActive ? "flex-1 bg-header" : "flex-1 bg-screen"}>
        <ScrollView
          ...
          contentContainerStyle={{
            ...
            paddingTop: NATIVE_LIQUID_GLASS_SUPPORTED ? insets.top + 72 : 24,
          }}
        >
