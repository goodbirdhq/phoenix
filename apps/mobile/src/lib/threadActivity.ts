...
function isProviderRetryTerminalError(
  ...
  if (next?.sourceActivityKind !== "runtime.error") {
    return false;
  }
...
export function buildThreadFeed(
  ...
  const unsettledTurnId = deriveUnsettledTurnId(thread.latestTurn ?? null);
  const activityEntries = getThreadFeedActivityEntries(thread.activities)
    ...
    .map((entry) => applyProviderRetryState(entry, unsettledTurnId));
...
function toThreadFeedActivityEntry(
  ...
  return {
    ...
    activity: {
      ...
      summary,
      detail,
      canExpand: workEntryCanExpand(entry) || workEntryHasExpandedBody(entry),
      getFullDetail,
      getCopyText,
...
/**
 * Provider retries are the only activity whose presentation depends on
 * whether the thread's turn is still live, so they cannot be baked into the
 * activities-keyed cache: recompute the summary/detail/expansion for the
 * current turn without rebuilding the whole cached entry.
 */
function applyProviderRetryState(
  entry: Extract<RawThreadFeedEntry, { readonly type: "activity" }>,
  unsettledTurnId: TurnId | null,
): Extract<RawThreadFeedEntry, { readonly type: "activity" }> {
  const workEntry = entry.activity.workEntry;
  const retry = workEntry.providerRetry;
  if (!retry) return entry;
  const retryState = providerRetryState(retry, {
    followedByActivity: retry.followedByActivity,
    turnInProgress: entry.turnId !== null && entry.turnId === unsettledTurnId,
  });
  const summary = providerRetrySummary(retry, retryState);
  const detail = retry.messages.at(-1) ?? null;
  const getFullDetail = memoizeValue(() =>
    retry.messages.length > 1 ? providerRetryDetail(retry) : buildWorkEntryExpandedBody(workEntry),
  );
  const getCopyText = memoizeValue(() =>
  ...
  return {
    ...
    activity: {
      ...
      summary,
      detail,
      canExpand: retry.messages.length > 1,
      getFullDetail,
      getCopyText,
    },
  };
