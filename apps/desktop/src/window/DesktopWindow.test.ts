...
vi.mock("electron", async (importOriginal) => ({
  ...
  session: {
    fromPartition: vi.fn(() => ({
      getUserAgent: vi.fn(() => "Mozilla/5.0 Electron/41.5.0 phoenix/1.2.3"),
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.mock(PreviewManager.PreviewManager)({
          ...
          isBrowserPartition: (partition) => partition.startsWith("persist:phoenix-preview-"),
          getBrowserPartition: () => Effect.succeed("persist:phoenix-preview-test"),
          reapplyZoom: () =>
            Effect.sync(() => {
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "phoenix://app/",
        navigationUrl: "phoenix://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it.effect("does not open a development window until the backend is ready", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], ["phoenix-dev://app/"]);
  ...
  it.effect("recovers when the development renderer is temporarily unreachable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        didFailLoad({}, -9, "ERR_UNEXPECTED", "phoenix-dev://app/", true);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "phoenix-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "phoenix-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
