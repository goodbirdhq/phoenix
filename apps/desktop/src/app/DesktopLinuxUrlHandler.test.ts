...
const makeEnvironment = (overrides: Record<string, unknown> = {}) =>
  DesktopEnvironment.DesktopEnvironment.of({
    ...
    isPackaged: true,
    isDevelopment: false,
    displayName: "Phoenix (Alpha)",
    linuxDesktopEntryName: "phoenix.desktop",
    linuxWmClass: "phoenix",
...
describe("DesktopLinuxUrlHandler", () => {
  it("renders a scheme-handler desktop entry with freedesktop Exec quoting", () => {
    ...
    assert.include(entry, "MimeType=x-scheme-handler/phoenix;");
  });
  ...
  it("carries structured context on registration errors", () => {
    const writeError = new DesktopLinuxUrlHandler.DesktopLinuxUrlHandlerRegistrationError({
      ...
      scheme: "phoenix",
      desktopEntryPath: "/home/alice/.local/share/applications/phoenix.desktop",
      cause: new Error("boom"),
    });
    assert.equal(
      writeError.message,
      "Failed to register the phoenix:// URL handler (step: write-desktop-entry).",
    );
    assert.equal(
      writeError.desktopEntryPath,
      "/home/alice/.local/share/applications/phoenix.desktop",
    );
    ...
    assert.equal(
      exitError.message,
      "Failed to register the phoenix:// URL handler (step: set-default-handler, xdg-mime exit code 4).",
    );
  });
  ...
  it.effect("writes the handler entry and claims the scheme default via xdg-mime", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.include(recorded.files[0]?.content, "MimeType=x-scheme-handler/phoenix;");
      assert.deepEqual(recorded.commands, [
        {
          command: "xdg-mime",
          args: ["default", "phoenix.desktop", "x-scheme-handler/phoenix"],
        },
      ]);
    });
  });
  ...
  it.effect("writes the portal identity without claiming the URL scheme in development", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(unpackaged, {
        environment: {
          isPackaged: false,
          linuxDesktopEntryName: "phoenix-dev.desktop",
        },
      });
      ...
      assert.equal(
        ...
        "/home/alice/.local/share/applications/phoenix-dev.desktop",
      );
    ...
    return Effect.gen(function* () {
      ...
      yield* runRegister(writeFailed, {
        writeError: PlatformError.systemError({
          ...
          pathOrDescriptor: "/home/alice/.local/share/applications/phoenix.desktop",
        }),
      });
