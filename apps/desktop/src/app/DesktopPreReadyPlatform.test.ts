...
describe("DesktopPreReadyPlatform", () => {
  ...
  for (const previousEntry of [undefined, 'Exec="/Applications/deleted-previous.AppImage" %U']) {
    it.effect(
      ...
      () => {
        ...
        let desktopName = "phoenix.desktop";
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/phoenix.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "phoenix.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=Phoenix (Alpha)");
            assert.include(identity.desktopEntry ?? "", "MimeType=x-scheme-handler/phoenix;");
          }),
  ...
  it.effect("acquires a synchronous pre-ready layer before an asynchronous later layer", () =>
    Effect.gen(function* () {
