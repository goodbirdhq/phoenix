...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/Phoenix.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/Phoenix.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.goodbird.phoenix.dev");
      assert.equal(environment.linuxWmClass, "phoenix-dev");
      assert.equal(environment.linuxDesktopEntryName, "phoenix-dev.desktop");
      assert.deepEqual(
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "phoenix.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.phoenix/dev");
      assert.equal(production.stateDir, "/Users/alice/.phoenix/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " com.goodbird.phoenix.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.goodbird.phoenix.dev.local");
    }),
  );
