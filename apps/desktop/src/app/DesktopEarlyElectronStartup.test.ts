...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("reads the persisted linux password-store preference before Electron is ready", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: { PHOENIX_HOME: "/home/user/.phoenix-test" },
      homeDirectory: "/home/user",
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.phoenix-test/userdata/desktop-settings.json");
  ...
  it("resolves the early linux Electron switches", () => {
    const options = resolveEarlyLinuxElectronOptions({
      env: {
        PHOENIX_HOME: "/home/user/.phoenix-test",
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "phoenix-dev",
      linuxDesktopEntryName: "phoenix-dev.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.phoenix/dev when PHOENIX_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.phoenix/dev/desktop-settings.json");
  ...
  it("treats whitespace-only PHOENIX_HOME as unconfigured in development", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        PHOENIX_HOME: "   ",
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
