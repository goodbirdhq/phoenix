...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.PhoenixSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    const command = [
      ...
      "--object-path",
      "/com/goodbird/Phoenix/SnapShot",
      "--method",
      "com.goodbird.Phoenix.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        ...
        path: "/com/goodbird/Phoenix/SnapShot",
        interface: "com.goodbird.Phoenix.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
