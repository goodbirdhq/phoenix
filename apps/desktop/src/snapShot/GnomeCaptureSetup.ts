...
export async function installGnomeCaptureBundle({ bundle, dataHome }: SetupPaths) {
  ...
  if (existing) {
    ...
    if (installed.version > metadata.version)
      throw new Error("A newer extension is installed. Update Phoenix instead of replacing it.");
  }
  ...
  try {
    ...
    if (existing) {
      const backupParent = NodePath.join(dataHome, "phoenix", "extension-backups");
    ...
    try {
      ...
      if (installed && (!info.state || (info.version && installed.version > info.version.value)))
        return {
          ...
          message:
            "Installed. Save your work, sign out of GNOME and sign back in, then return here to enable the extension. Restarting Phoenix alone is not enough.",
        };
      ...
      if (!properties.UserExtensionsEnabled.value)
        return {
          ...
          message:
            "GNOME has disabled user extensions. Turn on Extensions in the GNOME Extensions app, then check again. Phoenix will not enable your other extensions for you.",
        };
      if (info.state?.value === 1)
        return {
          status: "enabled",
          message: "The Phoenix extension is running. Active-window snapshots are available.",
        };
      ...
      return {
        status: "disabled",
        message:
          "Enable the Phoenix extension to allow active-window snapshots. You can disable it here at any time.",
      };
    } catch (error) {
      return {
        status: "error",
