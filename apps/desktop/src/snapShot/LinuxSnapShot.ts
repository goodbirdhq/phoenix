...
const EXTENSION = "org.gnome.Shell.Extensions.PhoenixSnapShot";
const EXTENSION_PATH = "/org/gnome/Shell/Extensions/PhoenixSnapShot";
...
export class LinuxCaptureConnection {
  ...
  async captureExtension(appId: string, options?: FeedbackOptions): Promise<LinuxWindowSnapshot> {
    ...
    if (result !== RequestNameReply.PRIMARY_OWNER) {
      throw new Error("Another Phoenix instance is capturing a window. Try again.");
    }
