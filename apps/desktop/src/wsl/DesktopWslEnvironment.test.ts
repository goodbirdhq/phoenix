...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    ...
    expect(script).toContain('runtime_parent="$HOME/.phoenix/wsl-runtime"');
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.phoenix/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.phoenix/wsl-runtime/1.2.3-x64",
    );
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain(
      'rm -f "$HOME/.phoenix/wsl-runtime/1.2.3_x64/.t3code-wsl-runtime-ready"',
    );
...
describe.skipIf(posixShellRunner === null)("WSL runtime install script (executed)", () => {
  ...
  const createFixture = () => {
    ...
    return {
      ...
      serverEntry: `${work}/home/.phoenix/wsl-runtime/${runtimeId}/apps/server/dist/bin.mjs`,
      installScript,
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.phoenix/wsl-runtime"',
