// This file mostly exists because we want dev mode to say "Phoenix (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
const APP_DISPLAY_NAME = isDevelopment ? "Phoenix (Dev)" : "Phoenix (Alpha)";
const APP_BUNDLE_ID = isDevelopment
  ? `com.goodbird.phoenix.dev.${devBundleIdSuffix || "local"}`
  : "com.goodbird.phoenix";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["phoenix-dev"] : ["phoenix"];
// Bumped past both lineages' prior values so every upgrading install rebuilds
// its cached mac launcher bundle at least once after this sync.
...
export function makeDevelopmentEnvironmentScript(environment) {
  const envEntries = [
    ...
    ["PHOENIX_PORT", environment.PHOENIX_PORT],
    ["PHOENIX_HOME", environment.PHOENIX_HOME],
...
export function resolveMacBundleInfoPlistStrings(executableName) {
  return {
    ...
    NSScreenCaptureUsageDescription:
      "Phoenix captures the active window when you use the snapshot shortcut.",
    NSDocumentsFolderUsageDescription: "Phoenix reads project files you open in the desktop app.",
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the Phoenix name.
