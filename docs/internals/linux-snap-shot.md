...
`<app-id>.SnapShot` on the session bus and exports `com.goodbird.Phoenix.SnapShot.Capture`; the config
...
Source in `apps/desktop/gnome-extension`, UUID `snap-shot@phoenix.codes`. GNOME only discovers a newly
...
The extension trusts callers that own `phoenix.SnapShot` (or the `phoenix-dev.SnapShot`
