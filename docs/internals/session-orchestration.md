...
- **Capability guidance** — `apps/server/src/provider/SessionOrchestrationInstructions.ts` is
  ...
  Phoenix MCP configuration. The block states whether `enableSessionOrchestration` is on or off,
  explicitly superseding previous guidance retained in provider history. This shared path covers Codex,
  ...
  provider session. Settings read failures use the disabled guidance; missing MCP configuration
  and inputs too close to the provider input limit omit the guidance. It prompts a concrete proposal and provider discovery
