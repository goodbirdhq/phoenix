...
parent's explicit call. It refuses a `starting`/`running` child with an actionable message rather
than stopping it (killing a working agent stays a deliberate `stop_session`), and otherwise
dispatches `thread.settle`, which keeps the decider's own guards for open approvals and queued
turns.
...
is kept and reported. The result always names what was removed and what was kept, and the thread's
`worktreePath` is cleared so `read_session` stops advertising a directory that no longer exists.
