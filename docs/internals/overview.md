...
Phoenix is a server runtime that owns agent sessions, workspaces, and version control, plus clients
...
there, never in the client. A remote client must never substitute its own filesystem, provider
credentials, or machine state for the environment's. The desktop app bundles a server, but its
renderer follows the same boundary.
