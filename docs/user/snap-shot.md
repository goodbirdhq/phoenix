...
Shift keys together. Phoenix attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Phoenix is in front captures Phoenix itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Phoenix refuses shortcuts
...
terminal contents. If an app is slow to answer, Phoenix attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Phoenix SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Phoenix under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Phoenix shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Phoenix shows the exact binding it will
...
**Other Wayland desktops.** Phoenix uses the desktop's screenshot portal when it can capture the
