...
The same configuration lives in `~/.phoenix/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Phoenix writes the built-in defaults into that file on
