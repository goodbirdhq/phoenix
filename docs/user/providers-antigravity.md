...
Phoenix runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Phoenix to confirm
...
hostname. Only that Phoenix sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Phoenix's separate Plan mode
...
Phoenix keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Phoenix also reads `.gemini/skills` and the
