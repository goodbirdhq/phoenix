...
The Phoenix web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Phoenix shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                  |
| -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Phoenix Linux background service. Select the button and leave Phoenix open while it prepares, tests, restarts, and reconnects.                            |
| **Update the desktop app** | Open the Phoenix desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
...
`phoenix@<version> service update` command on the server machine. That one local update installs the
...
**Copy update command** gives you `phoenix@<client-version>`, which relaunches the server directly
...
```sh
phoenix@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `phoenix service update`
...
3. For a command-line server, relaunch it with `phoenix@<client-version>`, replacing
