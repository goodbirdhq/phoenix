...
Both instances must use the same **CODEX_HOME path**. Phoenix prepares the shared
...
Choose the other account from the thread's model picker. Phoenix offers compatible
...
reports them. If Codex does not report either value, Phoenix leaves it out instead of using the
