...
This guide is for people who want to use more than one Codex account in Phoenix. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Phoenix](./install.md).
...
In Phoenix Settings, name it something obvious:
...
In Phoenix Settings, add another Codex provider:
...
Phoenix shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Phoenix treats it
