...
This guide is for people who want to use more than one Claude setup in Phoenix. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Phoenix](./install.md).
...
In Phoenix Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Phoenix uses Claude Code's normal config directory.

When you set this field, Phoenix points Claude Code at that directory with the
...
Phoenix looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where Phoenix looks.

Then add another Claude provider in Phoenix:
...
Phoenix only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Phoenix keeps separate config directories isolated
...
Add or edit a Claude provider in Phoenix Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Phoenix stores the value as a server secret and does not
...
Phoenix does not need a special Claude Code Router provider. Treat the router as a Claude
