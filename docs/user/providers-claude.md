...
Phoenix uses Claude Code's login and configuration. Start with the default provider
...
the rest of your environment stay as they are. Use the same variable for the login command; setting
`HOME` instead can put credentials where this provider will not find them.
...
Phoenix uses the Claude configuration on the connected server.
