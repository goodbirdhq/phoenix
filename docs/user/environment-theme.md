...
machine running your Phoenix server does that, its themes appear in the theme library alongside
...
different theme in Settings afterwards sticks until the next `phoenix theme set`, and running the same
set again is how you bring clients back. `phoenix theme clear` removes the setting without changing
what anyone currently has, and `phoenix theme show` prints the current theme and everything the
machine publishes. Only the environment you are anchored to publishes themes, so a remote client
follows the machine it is connected to, not the device it runs on. Phoenix Mobile keeps its own
...
the standard Phoenix look. A theme you saved always wins over a published one with the same id.
...
A machine publishes themes by writing files into the `themes` directory of the Phoenix state
directory (`~/.phoenix/userdata/themes/` by default). The filename is the theme id — `nightfall.json`
...
exported from Phoenix (the **Download** button's output) works as-is, so any theme someone shared
...
`appearance` is `light` or `dark`, and `canvas` and `accent` are hex colors. Phoenix generates the
rest of the palette from those two, the same way the guided theme editor does, so the result reads
as a Phoenix theme wearing your desktop's colors rather than a transplant of another app's.
...
out keep their generated value. A role this version of Phoenix does not know is ignored rather
...
Write each file atomically — write a temporary file beside it and rename — so Phoenix never reads
