...
Phoenix selects a project icon automatically. It checks `t3.json`, common favicon and app icon
...
Phoenix supports SVG, PNG, ICO, JPEG, GIF, AVIF, and WebP files. The selected path applies to
...
Phoenix only pulls when it can fast-forward and the checkout has no changed files, untracked files,
