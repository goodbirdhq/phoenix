...
off, whose past activity is still yours. OpenCode history comes from its local SQLite usage store;
instance-specific `XDG_DATA_HOME` and `OPENCODE_DB` settings are respected. Two environments that
share one machine still count that machine's history once.
