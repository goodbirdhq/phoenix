...
Phoenix uses the OpenCode setup on the connected environment. With a remote environment, its
OpenCode login and configuration apply, not the setup on your desktop or phone.

Phoenix requires OpenCode 1.14.19 or newer. It checks the server version before it loads models or
...
that environment value for both the local process and Phoenix.

With a server URL, Phoenix connects to that external server and uses only the password in the
...
After changing an OpenCode login or configuration outside Phoenix, open **Settings > Providers**,
select the environment, and choose **Refresh provider status**. Changing the provider's
configuration in Phoenix also replaces that provider connection.
...
Phoenix does not own an external OpenCode server. Native configuration changes on that server can
require its own reload or restart before a refresh returns the new list.

If a refresh fails, Phoenix keeps the last known models, slash commands, and skills. Fix the
...
until the model metadata returns. Phoenix does not switch the thread to the first model in the
list.
