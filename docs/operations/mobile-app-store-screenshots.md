...
the software keyboard. The environment cards come from real paired servers.
...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates Phoenix, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Phoenix review diff.
...
The Pending rows use the production offline outbox and point at the real Phoenix and React fixture
