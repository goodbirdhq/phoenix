...
- App: `Phoenix Dev`
- Bundle/package identifier: `com.goodbird.phoenix.dev`
- URL scheme: `phoenix-dev`
...
Run backend commands from the repository root. Use the ignored, worktree-local `.phoenix` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.phoenix` state.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `phoenix-dev`.
...
- Bundle ID: `com.goodbird.phoenix.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.goodbird.phoenix.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.goodbird.phoenix.dev
...
```text
phoenix-dev://connections/new?pairingUrl=<encoded-pairing-url>&autoConnect=1
```
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in Phoenix when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from Phoenix Dev.
